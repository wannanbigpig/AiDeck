    ...
    >
      <div className='export-json-toolbar'>
        <button
          ...
          disabled={!hasContent}
        >
          {showFullPreview ? <><EyeSlashIcon size={14} style={{ marginRight: 6 }} /> 隐藏</> : <><EyeIcon size={14} style={{ marginRight: 6 }} /> 预览</>}
        </button>
        <button className='btn btn-sm export-json-toolbar-btn' type='button' onClick={handleCopy} disabled={!hasContent}>
          {copied ? <CheckIcon size={14} style={{ marginRight: 6 }} stroke='#10b981' /> : <CopyIcon size={14} style={{ marginRight: 6 }} />}
          复制
        </button>
        <button className='btn btn-sm btn-primary export-json-toolbar-btn' type='button' onClick={onDownload} disabled={!hasContent}>
          <ArrowDownTrayIcon size={14} style={{ marginRight: 6 }} /> 下载
        </button>
      </div>
