...
test('账号索引缺条目时应从详情文件修复而不是丢账号', () => {
  resetStorageDir()
  storage.initStorage()

  const first = storage.addAccount('codex', {
    email: 'index-a@example.com',
    account_id: 'acc-index-a',
    organization_id: 'org-index',
    tokens: { access_token: 'at-a', refresh_token: 'rt-a' }
  })
  const second = storage.addAccount('codex', {
    email: 'index-b@example.com',
    account_id: 'acc-index-b',
    organization_id: 'org-index',
    tokens: { access_token: 'at-b', refresh_token: 'rt-b' }
  })
  assert.equal(storage.listAccounts('codex').length, 2)
  ...
  revisionBus.touchRevision('test-stale-index', { platform: 'codex' })

  const accounts = storage.listAccounts('codex')
  assert.equal(accounts.length, 2)
  assert.equal(accounts.some((account) => account.id === first.id), true)
  assert.equal(accounts.some((account) => account.id === second.id), true)

  const repairedIndex = fileUtils.readJsonFile(indexFile)
  assert.equal(repairedIndex.accounts.length, 2)
})
