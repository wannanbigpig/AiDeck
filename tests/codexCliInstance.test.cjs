...
test('Codex CLI 绑定实例应复用 CODEX_HOME、隔离会话且不切换当前账号', async () => {
  ...
  try {
    ...
    fs.writeFileSync(path.join(process.env.CODEX_CONFIG_DIR, 'config.toml'), [
      '[mcp_servers.docs]',
      'command = "node"',
      'args = ["server.js"]'
    ].join('\n') + '\n', 'utf8')
    ...
    assert.equal(fs.lstatSync(path.join(first.env.CODEX_HOME, 'config.toml')).isSymbolicLink(), true)
    assert.match(fs.readFileSync(path.join(first.env.CODEX_HOME, 'config.toml'), 'utf8'), /\[mcp_servers\.docs\]/)
