 ...
 * 准备 OAuth 会话（手动回调模式）
 */
function prepareOAuthSession (port) {
  try {
...
async function completeOAuthSession (sessionId, callbackUrl) {
  ...
  const callbackRaw = (callbackUrl || '').trim()
  if (!callbackRaw) return { success: false, error: '请粘贴完整回调地址' }
  ...
  try {
    if (refreshTokenValue) {
      ...
      if (!res.ok || !res.data || !res.data.access_token) {
        if (!nextAccessToken) {
          const quotaError = _extractGeminiQuotaError(res.status, 'Token 刷新失败: ' + (res.raw || '').slice(0, 200))
          const persisted = _persistGeminiQuotaError(accountId, quotaError)
          return {
            success: false,
        ...
        nextAccessToken = res.data.access_token
        nextExpiryDate = nowMs + expiresIn * 1000
        tokenRefreshed = true
        if (res.data.id_token) nextIdToken = res.data.id_token
        if (res.data.token_type) nextTokenType = res.data.token_type
        if (res.data.scope) nextScope = res.data.scope
      }
    }
