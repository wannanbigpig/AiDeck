...
export default function CodexSettingsModal ({ open, onClose, toast, settings: outerSettings, onSettingsChange, svc }) {
  ...
  const resolvedCodexStartupPath = settings.codexStartupPath || svc?.getDefaultCodexAppPath?.() || ''
  const resolvedOpenCodeStartupPath = settings.openCodeStartupPath || svc?.getDefaultOpenCodeAppPath?.() || ''
  ...
  useEffect(() => {
    if (open) {
      ...
      if (saved) {
        setSettings(normalizeCodexAdvancedSettings(saved))
      } else {
  ...
  const handlePickAppPath = ({ key, title }) => {
    if (!window.utools) {
