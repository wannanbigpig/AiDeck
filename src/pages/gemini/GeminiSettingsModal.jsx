...
export default function GeminiSettingsModal ({ open, onClose, toast, settings: outerSettings, onSettingsChange }) {
  ...
  useEffect(() => {
    if (open) {
      ...
      try {
        if (window.utools) {
          saved = window.utools.dbStorage.getItem('gemini_advanced_settings')
        } else {
          const s = localStorage.getItem('gemini_advanced_settings')
  ...
  const handleChange = (key, val) => {
    ...
    setSettings(next)
    if (window.utools) {
      window.utools.dbStorage.setItem('gemini_advanced_settings', next)
    } else {
      localStorage.setItem('gemini_advanced_settings', JSON.stringify(next))
    }
