...
export default function AntigravitySettingsModal({ open, onClose, toast, settings: outerSettings, onSettingsChange, svc }) {
  ...
  useEffect(() => {
    ...
    try {
      if (window.utools) {
        saved = window.utools.dbStorage.getItem('antigravity_advanced_settings')
      } else {
        const raw = localStorage.getItem('antigravity_advanced_settings')
        if (raw) saved = JSON.parse(raw)
      }
    } catch (e) { }
    if (saved) {
      setSettings(normalizeAntigravityAdvancedSettings(saved))
    } else {
  ...
  const handleChange = (key, val) => {
    ...
    try {
      if (window.utools) {
        window.utools.dbStorage.setItem('antigravity_advanced_settings', next)
      } else {
        localStorage.setItem('antigravity_advanced_settings', JSON.stringify(next))
      }
    } catch (e) { }
  ...
  const resolvedStartupPath = settings.startupPath || svc?.getDefaultAntigravityAppPath?.() || ''
